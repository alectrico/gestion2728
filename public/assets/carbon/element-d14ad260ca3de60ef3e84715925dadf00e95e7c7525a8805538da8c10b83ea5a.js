...
(function() {
  ...
  Carbon.Element = function(config) {
  ...
  Carbon.Element.prototype.registerProperty = function(key, def, config) {

    if (this._properties[key] === undefined) {
      this._properties[key] = new Carbon.Property(this, key, def);
    } else {
    ...
    if (config instanceof Array && config[0] instanceof Carbon.Element && typeof config[1] === 'string') {
      if (!config[0]._ready) {
    ...
    if (!this.hasOwnProperty(key)) {
      ...
      Object.defineProperty(this, key, {
        ...
        set: function(value) {
          ...
          if (_oldValue instanceof Carbon.Element) {
            _oldValue._parents.splice(_oldValue._parents.indexOf(parentRef), 1);
            delete this._children[key];
          }

          if (value instanceof Carbon.Element) {
  ...
  Carbon.Element.prototype.registerProperties = function(properties) {
  ...
  Carbon.Element.prototype.bindProperty = function(key, target, targetkey) {
    ...
    if (target instanceof HTMLElement) {
      ...
      target._properties[targetkey] = new Carbon.Property(target, targetkey, {});
    }
  ...
  Carbon.Element.prototype.addEventListener = function(type, listener) {
  ...
  Carbon.Element.prototype.removeEventListener = function(type, listener) {
    ...
    console.warn('Carbon.Element: Could not find listener', type);

  };

  Carbon.Element.prototype.removeEventListeners = function(listener) {
  ...
  Carbon.Element.prototype.dispatchEvent = function(event) {
  ...
  Carbon.Element.prototype.debounce = function(id, callback, timeout) {
  ...
  Carbon.Element.prototype.dispose = function() {
